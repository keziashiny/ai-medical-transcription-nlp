...
# ---- Paths (relative, good for Streamlit Cloud) ----
...
st.markdown(
    """
This app lets you explore a medical transcription dataset using **semantic search** and a **chat-style assistant**.  
...
`"fever and chills"`, `"abdominal pain"`, `"shortness of breath"`, `"postoperative chest pain"`).
    ...
    top_k_search = st.sidebar.slider("Number of results (search mode)", min_value=3, max_value=15, value=5, step=1)
    ...
    example_search = st.sidebar.selectbox("Try an example:", ["(none)"] + example_queries, key="search_example")
        ...
        if not query_search.strip():
            ...
            if "medical_specialty" in results.columns:
                ...
                selected_specialty = st.selectbox(
                    "Filter by medical specialty (optional):", specialties, key="search_specialty"
                )
...
with chat_tab:
    ...
    if user_query:
        ...
        greeting_keywords = [
            "hi", "hello", "hey", "heyy", "hey there",
            "good morning", "good afternoon", "good evening",
            "how are you", "how r u", "how are u",
            "what's up", "whats up", "sup", "how's it going", "hows it going"
        ]

        smalltalk_keywords = [
            "i'm good", "im good", "i am good",
            "i'm fine", "im fine", "i am fine",
            "doing good", "doing fine", "not bad",
            "thank you", "thanks", "thank u", "thx"
        ]

        is_greeting = any(g in normalized for g in greeting_keywords)
        is_smalltalk = any(s in normalized for s in smalltalk_keywords)

        # ------- 1) Greeting response (no search) -------
        if is_greeting:
            assistant_reply = (
                "Hi! 👋 It's really nice to meet you.\n\n"
                "I'm your medical NLP assistant. I can help you explore this dataset of clinical notes — "
                "for example, you can ask things like:\n\n"
                "- `What are common causes of fever?`\n"
                "- `How is abdominal pain described?`\n"
                "- `What symptoms are associated with chest pain?`\n\n"
                "Whenever you're ready, just ask about a symptom, condition, or situation you're curious about."
            )
        ...
        # ------- 2) Small talk response (no search) -------
        elif is_smalltalk:
            assistant_reply = (
                "I'm glad to hear that 😊\n\n"
                "If you'd like, we can look at how similar situations are described in the clinical notes. "
                "You can try something like:\n\n"
                "- `show me notes about fever and chills`\n"
                "- `how do doctors describe shortness of breath?`\n"
                "- `what does the dataset say about chest pain?`"
            )
        ...
        # ------- 3) Normal semantic-search-based reply -------
        else:
            with st.chat_message("assistant"):
